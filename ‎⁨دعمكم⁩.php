...
if($update->callback_query ){
  if ($timer["acount"][$from_id] < time()) {
    if($update->callback_query->message->chat->id != $sudo and $update->callback_query->message->chat->id != 2023947389) {
$data = $update->callback_query->data;
$chat_id = $update->callback_query->message->chat->id;
$title = $update->callback_query->message->chat->title;
$message_id = $update->callback_query->message->message_id;
...
$admnb = [ 
  ...
  if($data == "settext"){
    if($chat_id == $sudo or $chat_id == 2023947389 ) {
      if(true){
        bot('EditMessageText',[
          'chat_id'=>$chat_id,
          'message_id'=>$message_id,
...
SETJSON($rshq);
  ...
  $chabot = $rshq['cha']; if ($chabot == null){$chabot = "NF_FN_NN";}
...
$RBEROO = [
  ...
  [['text'=>"☑️┇قناة البوت .",'url'=>"$chabot.t.me" ],['text'=>"📦┇الحساب .",'callback_data'=>"acc" ]],
   [['text'=>"📨┇الدعم الفني .",'url'=>"https://t.me/NF_FN_NN" ]]
   ]
  ];
$rshq = json_decode(file_get_contents("RSHQ/rshq.json"),true);
...
قناة وكيل بوت $nambot ( [@l_b10] - @NF_FN_NN ) القناة الرسميه لوكيل بوت $nambot لذالك لا يتوفر لدينا سوا هذه القنوات المطروحه اعلاه واذا توفر لدينا اي قناة سنقوم بنشرها في قنواتنا الرسميه ليكون لدى جميع المستخدمين العلم بذالك
...
'message_id'=>$message_id,
'text'=>"
*☑️┇يمكن شحن رصيدك في Ξ بوت يوسف  بطرق دفع عديدة. 👇
